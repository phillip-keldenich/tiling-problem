...
def segment_almost_equal(s1, s2, epsilon=1e-8):
    return (point_almost_equal(s1.local_start, s2.local_start, epsilon) and
            point_almost_equal(s1.local_end, s2.local_end, epsilon)) or \
           (point_almost_equal(s1.local_start, s2.local_end, epsilon) and
            point_almost_equal(s1.local_end, s2.local_start, epsilon))
...
class TileTypeDrawSegment(BaseModel, frozen=True):
    ...
    """
    local_start: tuple[float, float] = Field(..., description="Local start point of the segment")
    local_end: tuple[float, float] = Field(..., description="Local end point of the segment")
    ...
    def rotate(self, amount: int):
        ...
        return TileTypeDrawSegment(
            local_start=(x1 * cos_angle - y1 * sin_angle, x1 * sin_angle + y1 * cos_angle),
            local_end=(x2 * cos_angle - y2 * sin_angle, x2 * sin_angle + y2 * cos_angle)
        )
    ...
    def translate(self, amount: tuple[float, float]):
        ...
        dx, dy = amount
        return TileTypeDrawSegment(
            local_start=(x1 + dx, y1 + dy),
            local_end=(x2 + dx, y2 + dy)
        )
...
class TileTypeDrawVertex(BaseModel, frozen=True):
    ...
    location: tuple[float, float] = \
        Field(..., description="Location of the vertex (origin is at the center of the tile)")
...
class TileTypeDrawing(BaseModel):
    segments: list[TileTypeDrawSegment] = \
        Field(..., description="List of segments in the tile type")
    vertices: list[TileTypeDrawVertex] = \
        Field(..., description="List of vertices in the tile type")
...
class TileType(BaseModel):
    ...
    bottom_edges: list[float] = Field(..., description="List of bottom edge indices of the tile type")
    top_edges: list[float] = Field(..., description="List of top edge indices of the tile type")
    left_edges: list[float] = Field(..., description="List of left edge indices of the tile type")
    right_edges: list[float] = Field(..., description="List of right edge indices of the tile type")
    drawing: TileTypeDrawing = Field(..., description="Drawing information of the tile type")
    ...
    def rotate(self, amount: int):
        ccw_order = [self.bottom_edges, self.right_edges, self.top_edges, self.left_edges]
...
class TilingInstance(BaseModel):
    ...
    tile_types: list[TileType] = Field(..., description="List of tile types in the tiling instance")
    tile_type_counts: list[int] = Field(..., description="List of counts for each tile type")
    ...
    def check_counts(self):
        if len(self.tile_types) != len(self.tile_type_counts):
            raise ValueError("Tile types and tile type counts must have the same length.")
